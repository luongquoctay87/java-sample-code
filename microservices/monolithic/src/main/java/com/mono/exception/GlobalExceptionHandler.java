...
public class GlobalExceptionHandler {
    ...
    public Error handleNullPointerException(BadCredentialsException e, WebRequest request, HttpServletResponse response) {
        Error error = new Error();
        error.setTimestamp(new Date());
        error.setPath(request.getDescription(false).replace("uri=", ""));
    ...
    @ResponseStatus(NOT_FOUND)
    public Error handleNullPointerException(NullPointerException e, WebRequest request, HttpServletResponse response) {
        Error error = new Error();
        error.setTimestamp(new Date());
        error.setPath(request.getDescription(false).replace("uri=", ""));
        error.setStatus(NOT_FOUND.value());
        error.setError("Data is null");
        error.setMessages(e.getCause().toString());
